...
class PoolTankFoundationWidget(QWidget):
    ...
    def _format_result(self, r: dict) -> str:
        ...
        <body>
            ...
            <div class='card'>
                ...
                <div class='formula'>换填重量（仅统计，不计入抗浮总重量） = 换填体积 × 换填砂石重度 = {f3(v['V_cushion'])} × {f3(gamma_cushion)} = {f3(w['G_cushion'])} kN</div>
    ...
    def _on_export_triggered(self):
        ...
        try:
            ...
            add_text_paragraph(f"(10) 换填重量 G_cushion = V_cushion × gamma_cushion = {f3(volumes.get('V_cushion', 0.0))} × {f3(gamma_cushion)} = {f3(weights.get('G_cushion', 0.0))} kN（仅统计，不计入抗浮总重量）")
